...
export default function KasseForm({ stripeAktiv }: { stripeAktiv: boolean }) {
  const cart = useCart();
  ...
  const absenden = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!agb) { setFehler("Bitte akzeptiere AGB & Widerrufsbelehrung."); return; }
    ...
    try {
      ...
      if (data.stripeUrl) {
        // Weiter zur sicheren Stripe-Bezahlseite (Warenkorb wird nach Rückkehr/Erfolg geleert)
        window.location.href = data.stripeUrl;
        return;
      }
      // Vorkasse: Bestellung ist angelegt
      cart.clear();
    ...
    <div className="mx-auto max-w-5xl px-4 pt-6 pb-10">
      ...
      <div className="grid lg:grid-cols-[7fr_4fr] gap-8 items-start">
        <form onSubmit={absenden} className="card p-5 lg:p-6 space-y-4">
          ...
          {/* Zahlungshinweis: Stripe vs. Vorkasse */}
          <div className="rounded-[4px] bg-bg border border-line px-4 py-3 text-[13.5px] text-muted leading-relaxed">
            {stripeAktiv ? (
              <><b className="text-ink">Sichere Online-Zahlung:</b> Im nächsten Schritt bezahlst du
                sicher über unseren Zahlungspartner (Kreditkarte, Klarna u. a.). Deine Kartendaten
                erreichen uns nie direkt.</>
            ) : (
          ...
          {/* Pflicht: AGB / Widerruf */}
          <label className="flex items-start gap-2.5 text-[13px] text-ink leading-snug cursor-pointer">
            <input type="checkbox" checked={agb} onChange={(e) => { setAgb(e.target.checked); setFehler(""); }}
              className="mt-0.5 h-4 w-4 accent-bordeaux shrink-0" />
            <span>
              Ich akzeptiere die <Link href="/agb" target="_blank" className="underline text-gold-ink">AGB</Link> und habe die{" "}
              <Link href="/widerruf" target="_blank" className="underline text-gold-ink">Widerrufsbelehrung</Link> sowie{" "}
              <Link href="/datenschutz" target="_blank" className="underline text-gold-ink">Datenschutzerklärung</Link> zur Kenntnis genommen.
            </span>
          </label>
          ...
          <button disabled={sende} className="btn-gold w-full py-4 text-[16px] disabled:opacity-60">
            {sende ? "Wird übermittelt …" : stripeAktiv ? `Weiter zur Zahlung — ${euro(cart.gesamt)}` : `Zahlungspflichtig bestellen — ${euro(cart.gesamt)}`}
          </button>
