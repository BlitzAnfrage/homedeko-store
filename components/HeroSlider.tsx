...
export default function HeroSlider({ slides, onAr }: { slides: Slide[]; onAr?: () => void }) {
  ...
  return (
    <>
      {/* ══ MOBIL-HERO (< lg): kompakt, ein Bild, kein absolut-Chaos ══ */}
      <section className="hero-buehne lg:hidden">
        {/* Bild oben: Wohnszene mit schwebendem Motiv, feste saubere Höhe */}
        <div className="relative h-[300px] overflow-hidden">
          ...
          <Link href={s.href} className="absolute left-4 bottom-4 w-[38%] max-w-[150px]">
            <span className="block bg-white p-1.5 rounded-[5px]" style={{ boxShadow: "0 16px 30px -12px rgba(0,0,0,.5)" }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={s.motiv} alt={s.motivName} className="w-full aspect-square object-cover rounded-[3px]" />
            </span>
            <span className="mt-1.5 inline-block bg-white/95 text-ink-strong text-[11px] font-semibold px-2 py-1 rounded-[4px]">
              „{s.motivName}“ · <span className="text-bordeaux">ab {euro(s.ab)}</span>
            </span>
          </Link>
          {/* Slide-Punkte */}
      ...
      {/* ══ DESKTOP-HERO (≥ lg): Galerie-Szene mit Tiefe ══ */}
      <section className="hero-buehne relative overflow-hidden hidden lg:block">
        <div className="mx-auto max-w-7xl px-4 grid grid-cols-[46fr_54fr] items-center gap-4 min-h-[640px]">
          ...
          <div key={"t" + idx} className="stagger max-w-xl relative z-10 py-16">
            ...
            <p className="mt-4 text-[17px] text-muted max-w-md leading-relaxed">{s.sub}</p>
          ...
          <div className="relative h-[640px]">
            <div className="absolute inset-0 overflow-hidden rounded-l-[28px]">
              {slides.map((sl, i) => (
                /* eslint-disable-next-line @next/next/no-img-element */
                <img key={sl.szene} src={sl.szene} alt=""
            ...
            <Link key={"m" + idx} href={s.href}
              className="hero-reveal absolute left-2 bottom-16 w-[46%] max-w-[300px] group">
              <span className="hero-float block bg-white p-2.5 rounded-[6px]"
